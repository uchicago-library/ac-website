...
For information on how to generate a config file, please see [[config-generate][the
relevant section of our build instructions]].
...
| Arch Linux         | macOS                 | WSL Ubuntu           |
|--------------------+-----------------------+----------------------|
| =$ pacman -S opam= | =$ brew install opam= | =$ apt install opam= |
...
This will make the DLDC's very own =opam= repository available to you,
then install all of Attachment Converter's third-party library
dependencies.  =make deps= should suffice for most purposes, but if
you'd like to pursue the slightly fancier option of creating a
sandboxed =opam= switch, you can instead run:
...
*** Generate Config File <<config-generate>>
